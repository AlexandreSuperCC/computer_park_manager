...
public class MaterielManagerInterFrm3 extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MaterielManagerInterFrm3 frame = new MaterielManagerInterFrm3();
	 ...
	 * Create the frame.
	 */
	public MaterielManagerInterFrm3() {
		setTitle("consulter");
		setIconifiable(true);
		setClosable(true);
...
//			}
//		});
//		supprimerBtn.setIcon(new ImageIcon(MaterielManagerInterFrm3.class.getResource("/images/delete.png")));
...
//		changerBtn.addActionListener(new ActionListener() {
//			public void actionPerformed(ActionEvent e) {
//				changerActionPerformed(e);
//			}
//		});
//		changerBtn.setIcon(new ImageIcon(MaterielManagerInterFrm3.class.getResource("/images/change.png")));
...
//							.addComponent(supprimerBtn)
	...
	private synchronized void supprimerActionPerformed(ActionEvent evt) {
		...
		if(StringUtil.isEmpty(id)) {
			JOptionPane.showMessageDialog(null, "Selectionnez votre donnée ? supprimer");
			return;
		}
		Integer id_int = Integer.parseInt(id);
		
		int n = JOptionPane.showConfirmDialog(null, "Vous être sûr que vous voulez le supprimer?");
		
		Materiel materiel = new Materiel(id_int);
		if(n==0) {
			/*
			 * effectuer la liaision ? la base de données
			 */
			Connection con = null;
			try {
				con = dbUtil.getCon();
	...
	private void materielTableMousePressed(MouseEvent e) {
		...
		if("disponible".equals(materielTable.getValueAt(row, 4))) { //faire attention ? EQUALS() ici
	...
	/*
	 * Cette partie est pour modifier les caratéristiques d'un matériel 
	 */
	private synchronized void changerActionPerformed(ActionEvent evt) {
		// TODO Auto-generated method stub
		String id2 = this.idTxt.getText();
		if(StringUtil.isEmpty(id2)) {
			JOptionPane.showMessageDialog(null, "Selectionnez votre donnée ? modifier");
			return;
		}
		...
		/**
		 * Effectuer la liaison avec la base de donnée
		 */
		Connection con = null;
		try {
			con = dbUtil.getCon();
